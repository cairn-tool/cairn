...
Replace host-native substitutions with the canonical `${ARGUMENTS}`, `${BUNDLE_ROOT}`, and
`${SKILL_DIR}`. Convert any hand-written slash command into a skill with
