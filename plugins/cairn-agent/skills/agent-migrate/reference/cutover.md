...
An `agent.install` block beside it declares what _writes_ that tree, so the committed output is
generated by one command rather than copied by hand:
...
`--target` is repeatable and one destination may hold several installs, so a repository can
place two hosts' project trees in its root and verify both. Prefer this over `agent convert`
plus a copy: a copied tree carries no install manifest, so `agent verify` reports `AB426` and a
file the bundle stops rendering is never flagged.
...
- **Do not declare an entry for a target that claims a file you hand-maintain.** `AGENTS.md` is
  ...
  `AGENTS.md` will report drift forever, correctly — and installing one refuses with `AB801`,
  because the file exists and no install record accounts for it. A bundle with no `rules/`
  renders no `AGENTS.md`, so this only bites when you have one.
