...
| ------------------------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Shared options](common.md#common-options) | —       | `--format`, `--envelope`, `--provider`, `--project`, `--since`, `--until`, `--last`, `--top`, `--logs`, `--no-subagents`, `--no-index`, `--strict`, `-h`. |
...
Model and tool listings are capped at ten rows; use [`usage tokens`](tokens.md) and
[`usage tools`](tools.md) for the full picture.
...
- [`usage tokens`](tokens.md) breaks the same tokens down by model, time, project, or session.
- [`usage sessions`](sessions.md) reports the same activity one session at a time.
