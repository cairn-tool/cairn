...
`$XDG_DATA_HOME/opencode` when set, `~/.local/share/opencode` otherwise. One SQLite database
holds every session, so a transcript here is a `session` row rather than a file.

```text
opencode.db            sessions, messages, and parts
