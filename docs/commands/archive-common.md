...
version it ever saw without anyone asking it to. [`archive list`](archive-list.md) shows the
newest and counts the rest; [`archive extract`](archive-extract.md) reaches an older one by hash.
...
[`archive migrate`](archive-migrate.md).
...
Exit `2` comes only from [`archive verify`](archive-verify.md). It is the one command here that
