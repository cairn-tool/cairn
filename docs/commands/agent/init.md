...
[`agent convert`](convert.md) renders for Claude Code, Codex, and Cursor.
...
[`agent package`](package.md) requires real values. Keeping the two questions separate is what lets `agent init` run without human input.
