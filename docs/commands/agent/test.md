...
[`agent validate`](validate.md) answers "is this structurally valid?" and
[`agent doctor`](doctor.md) answers "does it still conform to the target profiles?".
...
render [`agent convert`](convert.md) would write. No model is called, no host tooling is
...
[`agent specs --format json`](specs.md) publishes the declared patterns per target.
...
[`agent audit`](audit.md)'s `audit.checks` does.
...
render was lossy" — unlike [`agent audit`](audit.md), no per-code split is needed. The
...
through [`agent specs`](specs.md) so you can run it yourself.
