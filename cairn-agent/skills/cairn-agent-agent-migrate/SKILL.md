---
name: cairn-agent-agent-migrate
...
when the surface genuinely has no portable equivalent. The `cairn-agent-portability-triage` skill decides
these case by case.
...
Replace host-native substitutions with the canonical `$ARGUMENTS`, `.`, and
`.`. Convert any hand-written slash command into a skill with
...
`cairn-agent-target-portability` explains how to read the result before promising a feature works
everywhere.
...
[`reference/cutover.md`](reference/cutover.md). Authoring is `cairn-agent-bundle-authoring`; per-host
support is `cairn-agent-target-portability`; publishing is `cairn-agent-bundle-publishing`.
