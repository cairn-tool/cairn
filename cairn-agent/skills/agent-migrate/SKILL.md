...
[`${PLUGIN_ROOT}/assets/cli-basics.md`](../../assets/cli-basics.md).

## 1. Survey, and touch nothing

Find the agent content in `the invocation arguments from the user's message` (or the current directory) and report a table of _path →
...
Replace host-native substitutions with the canonical `the invocation arguments from the user's message`, `${PLUGIN_ROOT}`, and
