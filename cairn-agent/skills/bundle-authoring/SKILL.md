...
| ---------------- | --------------------------------------------------- |
| `${PLUGIN_ROOT}` | `${PLUGIN_ROOT}`, or `${CLAUDE_PROJECT_DIR}` |
| `the invocation arguments from the user's message`   | `the invocation arguments from the user's message`, substituted natively                  |
| `.`   | `.`                               |
