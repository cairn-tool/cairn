...
| ---------------- | -------------------------------------------------------------- |
| `${PLUGIN_ROOT}` | The host's plugin-root variable, or its project-root variable. |
| `the invocation arguments from the user's message`   | The host's own substitution, natively where one exists.        |
