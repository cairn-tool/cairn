...
const CONTRACTS: ContractRow[] = [
  ...
  // Scripts
  {
    ...
    exitCodePassthrough: {
      ...
      max: 255,
      description:
        "In llm and human formats the script's own exit status is this process's exit status, verbatim; a script killed by a signal reports 128 + the signal number. --ignore-exit-code suppresses the passthrough entirely and exits 0.",
    },
    stream: { success: "stdout", findings: "stdout" },
    writes: false,
    stability: "experimental",
    notes:
      "The only command that executes anything. In llm and human formats the child inherits all three streams and its exit status passes through unchanged, so a hook reads the real code and this command writes nothing of its own to stdout; `exitCodes` describes --format json, which captures the streams into the payload instead. A script that never started exits 1 rather than 2, so a typo in exec[0] stays distinguishable from a script that ran and failed. The payload goes to stdout in every outcome, including a failed script, so a consumer never has to switch streams. Resolution walks every .cairn.yml from the working directory to the repository root and the nearest definition of the name wins; files under node_modules are skipped, and running outside a Git repository is refused unless --root sets the boundary. Configuration cannot change what executes: scripts commands accept no `commands:` defaults. The update notice is suppressed, because the child owns the real stderr. --ignore-exit-code exits 0 for every outcome, a refused resolution and a script that never started included: it exists for an invocation inline in a skill document, where a loader reads any non-zero status as a failure to load, so a script whose code carries meaning would otherwise keep the skill from loading. It changes only this process's status — the error text still reaches stderr, exit.status still carries the script's real code, and startupError is still emitted — which also means a mistyped name under that flag exits 0.",
  },
  {
    id: "scripts which",
    formats: BASE_FORMATS,
