...
agent
  ...
  .option("--layout <layout>", "Output layout: nested, release", "nested")
  .option("--no-source-bundles", "Do not publish the source bundles under the release layout")
  .option("--stamp-version <semver>", "Version written into each published agent-bundle.yaml")
  .option("--source-commit <sha>", "Commit recorded in release-manifest.json")
  .option("--readme <owner/name>", "Generate README.md for that repository's release branch")
