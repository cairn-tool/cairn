...
The repository is an npm workspace root that is itself the published CLI package. It also
publishes `@cairn-tool/agent-bundle-schema` from `packages/`; both go out at one version from
one semantic-release run.
...
src/agent/docs/*.ts    the bundle documentation artifacts: one payload builder, two artifacts
spec/v1/*.json         hand-written schemas for those artifacts; the source of truth
packages/*             the npm workspaces published beside the CLI
...
`spec/v1/` holds the hand-written schemas for the bundle documentation artifacts, and everything
downstream of them is generated by `npm run codegen`:

```text
spec/v1/_common.json                       definitions both artifacts share; a build input, never published
spec/v1/<artifact>.json                    an artifact schema, authored with $refs into _common
packages/agent-bundle-schema/schemas/      the same schemas with those definitions inlined -- GENERATED
packages/agent-bundle-schema/src/generated/types.ts   the model types -- GENERATED
```

**Do not edit anything under `packages/agent-bundle-schema/schemas/` or `src/generated/`.** Edit
the spec and regenerate. `npm run codegen:check` fails CI when the two have drifted, and it runs
in the `quality` job beside `typecheck`.

The inlining is not an optimization. A consumer validating an artifact should need exactly one
document, so each published schema carries its own copy of the shared definitions rather than a
cross-file `$ref` the consumer would have to resolve.

`src/agent/docs/` is typed as the _generated_ types, which is what makes `tsc` -- rather than a
test somebody remembered to write -- the thing that catches a payload drifting from the schema it
claims to conform to.

That is also why `typecheck` builds the workspace before it runs `tsc`. The CLI resolves the
schema package's types from its `dist/`, which `npm ci` does not produce -- it only links the
workspace. Without the build first, `typecheck` passes on a developer's machine, where `dist/` is
left over from the last build, and fails in CI on a clean checkout. `test` already built it
through `pretest`; `typecheck` was the one entry point that did not.
...
- **The edit guard is generated by `agent install`, per repository, and no plugin ships a hook
  that runs on every edit.** Through bundle 1.3.0 `cairn-agent` carried a global `pre-tool-use`
  hook that spawned `cairn agent guard` on every write in every repository — two Node startups
  (~0.16 CPU-s) per edit, on every machine with the plugin, mostly to learn the repository had
  opted into nothing — and under parallel agents that pinned machines. Now a project-scope
  install writes `.cairn-guard.sh` (`src/agent/install/guard-script.ts`), a POSIX `sh` script
  with the destination's inventory baked in as a `case` over exact paths, and registers it in
  each host's project hook document (`src/agent/install/hook-document.ts`). It forks nothing
  on the allow path. Do not reintroduce a hook in any bundle's `hooks.yaml` that calls the CLI
  per edit. The script must stay POSIX: CI's `/bin/sh` is dash. No `local`, arrays, `[[`,
  `${var//}`, `[[:space:]]` (older mawk), or backslashes in awk _replacement_ strings.
- **Guard records are rebuilt by every run and excluded from `assessDestination`'s owners.**
  The guard is tracked by `kind: "guard"` records named `.cairn-guard` (a name no bundle can
  have — `COMPONENT_NAME` forbids the dot). `finishGroup` drops the prior ones and re-plans from
  the merged document, so the script's bytes may change with every install without a sibling
  recording yesterday's sha — the `AB808` trap that co-owning it through bundle records would
  hit on a partial `--name` run. `parseRecord` rejects an unknown `kind` as `malformed`; an
  older cairn does not, and would read a guard record as a bundle whose inventory includes the
  user's `.claude/settings.json`, which is why `agent uninstall .cairn-guard` is refused
  (`AB813`) rather than routed anywhere. `agent uninstall` rebuilds the guard from the surviving
  bundles and retires it with the last one, counting non-guard records only.
- **The hook document is composed before assessment, replaced in place, merged never
  replaced, and guarded only when a bundle rendered it.** `.claude/settings.json` and
  `.cursor/hooks.json` are the user's (or `policies`'), so `planGuard` starts from the bundle's
  rendered document or the file on disk, finds its handler by command and replaces it _in
  place_ — remove-then-append would move it below a user-added handler and make `--check` and
  `agent verify` perpetually stale. A bundle draft that carried the document gets the composed
  bytes and a recomputed `record.files`, or the recorded sha describes bytes that never land.
  Retirement strips the handler and deletes the file only when `hookDocument.created` (carried
  forward run to run) and nothing else remains. The document is in the script's list only when
  a bundle's `policies` produced it.
- **Every byte of the guard derives from configuration, never argv, because `agent verify`
  reproduces it.** `regenerate` defaults to the fixed string `cairn agent install`:
  `tests/e2e/agent-verify.test.ts` installs without a config and verifies with one, and the two
  must agree. Verify passes the same `GuardSettings` into `planInstall`, then compares the guard
  once per destination (`verifyGuard`) rather than once per entry — sixteen entries share this
  repository's root. `AB810` is emitted by the install _command_, not the planner, for the same
  reason.
- **`sourcesFor` lives in `src/agent/guard/sources.ts`, a leaf, and the installer imports it
  from there.** `install/index.ts` → `guard/index.ts` → `guard/config.ts` → `install/index.ts`
  would be a cycle. The script's `sources` and `agent guard`'s must not drift:
  `tests/unit/agent-install-guard.test.ts` asserts them equal for every file an install writes.
- **`agent guard` still matches paths and never renders, and `matchOutputPattern` is why it
  can.** A `renderBundle` is ~400ms; the oracle is the target profile's declared `outputs`
  patterns plus an `fs.existsSync` against the bundle that sources the component the pattern's
  `{name}` names. That second half is the same lookup that produces the "edit this instead"
  pointer, and it is what stops a hand-written `.claude/agents/scratch.md` being refused.
  `outputPatternToRegExp` **stays non-capturing** — `import/detect.ts` compares its compiled
  source by identity against the declared pattern set — so the capturing form is a separate
  compiler beside it, and `tests/unit/agent-guard.test.ts` asserts the two agree over the whole
  profile matrix and that no cell declares two patterns describing one path. The feature key of
  an output pattern is the component key of a manifest, so the reverse map is `configuredPath`
  on the same key and no second table exists. The one place the guard must copy a renderer rule
  rather than share it is `marketplaceAssets`, because `render.ts` withholds `marketplace.icon`
  from the project profile; that test asserts those agree too.
